  ...
  return (
    <DesktopSettingPage title="Desktop – Paket" desc="Judul & deskripsi halaman paket desktop" keys={KEYS}>
      {({ get, set }) => (<>
        ...
        <ImageField settingsKey="desktop.bg_paket_left" label="Kiri — Background" hint="Warna, gradasi, atau upload gambar. Kosongkan = ikut default." get={get} set={set} />
        <ImageField settingsKey="desktop.bg_paket_right" label="Kanan — Foto (kosongkan = foto menu otomatis)" hint="Timpa foto panel kanan dengan gambar pilihan." get={get} set={set} />
      </>)}
    </DesktopSettingPage>
  )
}
