  ...
  return (
    <DesktopSettingPage title="Desktop – Minuman" desc="Judul & deskripsi halaman minuman desktop" keys={KEYS}>
      {({ get, set }) => (<>
        ...
        <ImageField settingsKey="desktop.bg_minuman_left" label="Kiri — Background" hint="Warna, gradasi, atau upload gambar. Kosongkan = ikut default." get={get} set={set} textInput textPlaceholder="#0D1410 / linear-gradient(...) / URL" />
        <ImageField settingsKey="desktop.bg_minuman_right" label="Kanan — Foto (kosongkan = foto menu otomatis)" hint="Timpa foto panel kanan dengan gambar pilihan." get={get} set={set} textInput textPlaceholder="#0D1410 / linear-gradient(...) / URL" />
      </>)}
    </DesktopSettingPage>
  )
}
